...
톰캣의 디렉터리는 대략 위와 같다. 가장 중요한 폴더는 `webapps` 라는 폴더이다. `webapps` 안에 디렉터리들이 하나의 애플리케이션으로 간주된다. 그래서 `webapps` 바로 아래 있는 폴더들의 이름을 각 application context 라고 한다. 톰캣이 실행 될 때 여기 webapps 안에 있는 애플리케이션들이 배포된다.
